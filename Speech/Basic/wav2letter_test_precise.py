...
def load_audio_scipy(audio_file, target_sr=16000):
    ...
    # Convert stereo to mono if needed (match librosa's method)
    ...
    # Use librosa's resampling if needed (more accurate than linear interpolation)
    if sample_rate != target_sr:
        print(f"DEBUG: Resampling from {sample_rate} Hz to {target_sr} Hz")
        try:
            import librosa
            audio_data = librosa.resample(audio_data, orig_sr=sample_rate, target_sr=target_sr)
            print("DEBUG: Using librosa.resample for accuracy")
        except ImportError:
            print("DEBUG: librosa not available, using scipy resampling")
            from scipy import signal
            # Use scipy's resample which is more accurate than linear interpolation
